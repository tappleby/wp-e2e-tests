...
export function startBrowser() {
	...
	if ( config.has( 'sauce' ) && config.get( 'sauce' ) ) {
		let caps = config.get( 'defaultSauceConfig' );
		let override = config.has( 'sauceConfig' ) ? config.get( 'sauceConfig' ) : {};

		// The override would be specific attributes provided in the NODE_CONFIG command line argument
		caps = _.extend( {}, caps, override );
